...
что файл (а os.Stdin является файлом) прочитан до конца. Чтобы ошибка была обработана правильно, вы можете поступить так:
...
func main() {
	...
	//fmt.Println(ss)
	var s1, s2 float64
	s1, _ = strconv.ParseFloat(ss[0], 64)
	s2, _ = strconv.ParseFloat(ss[1], 64)
	fmt.Printf("%.4f", s1/s2)
}
